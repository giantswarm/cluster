...
- Add ability to set a `RemidiationStrategy` for the `KubeadmControlPlane`. This can be configured by setting the `internal.advancedConfiguration.controlPlane.remidiationStrategy` value. In the future, `MachineHealthCheck.spec.maxUnhealthy` will be deprecated in favor of `RemidiationStrategy`.
