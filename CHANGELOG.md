...
- Migration hook: wrap kubectl invocations in a retry helper (`kubectl_retry`) that retries on transient API errors (`Unauthorized`, `connection refused`, `i/o timeout`, `TLS handshake`, `EOF`) with linear backoff up to 5 attempts. Under 5-cluster concurrent migration load the MC API server intermittently rejects requests with `Unauthorized` (transient auth-cache symptom); without retries, `set -eu` exits the hook script on the first such rejection and helm marks the upgrade failed, triggering a chart-operator retry storm. Each retry gives chart-operator on the WC time to come back up between Phase 0b scale-downs, defeating the protection. Retry-on-Unauthorized eliminates the storm so Phase 0b only needs to hold for a single hook execution.
- Migration hook: drop the 300s blocking wait at the end of Phase D. The bundle App CR deletes are still issued, but the hook no longer waits for the operator-driven cascade to fully complete before exiting. With `chart-operator` on the WC scaled to 0 (Phase 0b), the cascade cannot uninstall any helm releases on the WC during the upgrade, so it is safe to let the cascade finish in the background. The previous blocking wait could exceed helm's default 5-minute hook timeout, marking the upgrade failed and triggering a retry storm even when no application was actually being uninstalled.
- Migration hook: lift `spec.upgrade.timeout` on the cluster's own App CR to 15m in a new Phase 0a, before any other work happens. The default helm upgrade timeout enforced by `app-operator` / `chart-operator-MC` is 5 minutes; under fresh-state 5-cluster concurrent migration load (the realistic production scenario — fleet-wide GitOps release bumps), kubectl client-side throttling and MC API auth/throttle pushback push the hook's Phase A→C work past 5m, so helm marks the upgrade failed and retries. The patch only affects FUTURE retries (chart-operator-MC re-reads the App CR at the start of each helm upgrade), so the very first attempt may still hit the 5m default — but Phase 0b's `chart-operator` scale-down on the WC ensures that wasted first attempt cannot uninstall anything. Subsequent retries get a generous window and the migration converges. This makes the fix self-installing for any customer cluster regardless of how their App CR was authored, at the cost of one expected first-attempt timeout per migration.
- Migration hook: force-delete the `chart-operator` pod on the WC in Phase 0b instead of letting the `kubectl scale --replicas=0`-driven SIGTERM play out gracefully. Round 43 ran with the upgrade timeout pre-set to 15m (no retry storm, single hook execution) and still lost a bundle sub-app on 2/5 clusters. The remaining viable mechanism is the pod's `terminationGracePeriodSeconds` window: after the SIGTERM lands, chart-operator has up to 30s to drain its work queue, and any queued reconcile that resolves to `helm uninstall` (stale informer cache observing a finalizer/deletionTimestamp transition) fires DURING graceful shutdown. We now `kubectl scale --replicas=0` the deployment (so the ReplicaSet doesn't re-spawn) AND `kubectl delete pod --grace-period=0 --force` every existing chart-operator pod, terminating immediately and leaving no drain window.
- Migration hook: rewrite Phase D to **never trigger `helm uninstall` on the bundle helm releases on the MC**. Instead, pause the bundle App CRs + bundle Chart CRs on the MC in Phase A, strip their finalizers in Phase B, and delete them instantly in Phase D. Round 44 forensic logs (chart-operator-WC log captured continuously during the migration on 5 clusters) refuted the previous graceful-drain hypothesis — chart-operator-WC was idle at SIGTERM time, with `All workers finished` printed within microseconds of `Shutdown signal received`, and zero `helm uninstall` calls in 374 reconcile-update events. The actual root cause is a race between the OLD Phase D (which made `chart-operator-MC` run `helm uninstall` on the bundle release) and Flux helm-controller (which had already adopted that same bundle release via the new `HelmRelease` rendered by `cluster-aws`'s upgrade): Flux installed sub-app HRs as part of the bundle's manifest, then `chart-operator-MC`'s uninstall deleted those HRs as manifest items, and helm-controller's HR-deletion handler fired `helm uninstall` on each WC sub-app release. Keeping the bundle helm releases on the MC alive lets Flux adopt them cleanly, eliminating the manifest-deletion path that was causing all the observed sub-app uninstalls.
...
[Unreleased]: https://github.com/giantswarm/cluster/compare/v6.7.0...HEAD
...
[1.2.0]: https://github.com/giantswarm/cluster/compare/v1.1.0...v1.2.0
