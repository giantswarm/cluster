...
| `internal.advancedConfiguration.workers.localEphemeralStorage` | **Local ephemeral storage** - Cluster-wide parameters for the local instance-store NVMe feature. Enabled per node pool via `localNvme.enabled`; the bootstrap script mounts detected instance-store NVMe over the kubelet directory and adds a nodefs.available eviction backstop. No-op on nodes without instance store.|**Type:** `object`<br/>|
| `internal.advancedConfiguration.workers.localEphemeralStorage.evictionHardNodefsAvailable` | **nodefs.available eviction threshold** - Node-level disk-eviction backstop added on instance-store nodes only. An empty string skips the kubelet patch.|**Type:** `string`<br/>**Default:** `"10%"`|
| `internal.advancedConfiguration.workers.localEphemeralStorage.mountPath` | **Mount path** - Path the detected instance-store NVMe is mounted over. Defaults to the kubelet directory so pod emptyDir volumes land on the NVMe.|**Type:** `string`<br/>**Default:** `"/var/lib/kubelet"`|
