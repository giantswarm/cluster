...
| `global.components.containerd.enableOverridePath` | **Override path enabled** - override_path is used to indicate the host's API root endpoint is defined in the URL path rather than by the API specification. This may be used with non-compliant OCI registries which are missing the /v2 prefix. (Defaults to false)|**Type:** `boolean`<br/>**Default:** `false`|
